...
def vote_correspondences(
    ...
    *,
    ...
    视场最外角的位移仍有 3.07 px（该角位移的单轴分量是 2.17 px），对 `match_radius_px
    =5.0` 只有 **1.63x** 的余量（3.07 px 对应真实角位移，5.0/3.073≈1.63；单轴分量
    2.17 px 对应的是虚高的 2.30x）。均匀视场中在此旋转下位移超出 5.0 px 的星占比
