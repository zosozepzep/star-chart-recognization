...
class StarDetectorPipeline:
    def __init__(self):
        ...
        self.scorer = SourceScorer(acceptance_threshold=0.55) # 设置及格线
    ...
    def run(self, data: np.ndarray):
        ...
        candidates = self.generator.generate(data_sub, global_std) 
        ...
        # ==========================================
        # Step 2: 基础特征提取 (测光与背景估计)
        # ==========================================
        logger.info("Step 2: Extracting photometric features...")
        # ⚠️ 注意：这里必须改成 data_sub！
        candidates = self.extractor.extract(data_sub, candidates)

        # ==========================================
        # Step 3: PSF 拟合 (亚像素定位)
        # ==========================================
        logger.info("Step 3: Fitting 2D Gaussian PSF...")
        # ⚠️ 注意：拟合也必须在扣除了背景的 data_sub 上进行，否则基线会偏离
        candidates = self.fitter.fit_sources(data_sub, candidates)
