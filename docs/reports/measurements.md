...
低仰角跟踪下场在滚动——A 约 167 px/帧、B 约 119 px/帧。一个参考源要满足
「≥4 of 6 帧」至少要活过 3 步，即 A 501 px、B 357 px，占 4096 图幅宽的
12.2% / 8.7%。**滚出画幅的源无论探测多好都不可能复现**，所以复现率有一个
...
均以 `xfail(strict=True)` 隔离在 `tests/validate/test_repeatability.py` 里——
门限数字一字未动，一旦实测越过它，strict 会让测试转红强制重新裁决。
...
任务书草稿里的「103 vs 86」是**示意值**，不对应任何实测量，不得引用（裁决 286）。
