...
def test_dataset_b_zero_point_matches_baseline(dataset_b_dir):
    ...
    拟合散度比真实 σ **小约 3~4%，原因是 ``np.std`` 默认 ``ddof=0`` 加上样本标准差
    的有限 n 偏差，不是剪裁**。实测 400 次干净 55 点试验：比值在 σ 从 0.100 到
    0.500 的 5 倍跨度上基本恒定（实测 0.9821 / 0.9694 / 0.9823 / 0.9810），而
    剪裁数的中位数是 **0**。剪裁能解释的量会随 σ 变化，恒定比值只能是 ddof。
    n=55 时 ``sqrt((n-1)/n) = 0.9909``。
