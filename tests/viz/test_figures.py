...
# ---------------------------------------------------------------------------
# 尺寸下界。实测（容器内，matplotlib 3.10.9，dpi=150，11 档 figsize，
# 见 probe/t26-bytes.txt）：
#
#   figsize       纯空白   空坐标轴   薄真图(3 点折线)
#   7.2x7.2         7198     16063        35461
#  10.0x8.0         9930     18940        40053
#  12.0x5.0         7114     16075        34762
#   9.0x6.0         6927     15820        33791
#   8.0x6.0         6477     15388        33549
#   6.0x4.0         3552     12277        23643
#  12.0x6.0         8502     17523        38624
#  11.0x8.0        10677     19728        44355
#  12.0x8.0        11288     20396        44443
#  10.0x6.0         7492     16401        36006
#  14.0x6.0         9740     18775        40881
#
#   max 纯空白 = 11288    max 空坐标轴 = 20396    min 薄真图 = 23643
#
# 本模块实际只用 10.0x8.0、9.0x6.0、12.0x6.0 三档，它们的薄真图分别是
# 40053 / 33791 / 38624，所以对本模块可分离窗口是 **(20396, 33791)**，
# 取 **24000**（两侧都有余量）。
#
# 这条下界只挡「一张图基本没画东西」。它**挡不住**「画了中文标签但没画数据」的
# 退化图：实测（probe/t26-bytes2.txt）一张空坐标轴只要带上中文标题与轴标签就有
# 46983~59775 字节，远在任何合理下界之上。真正的防线是各图的 probe 口径断言。
MIN_REAL_FIGURE_BYTES = 24000   # 从 default.yaml 的 viz.min_real_figure_bytes 读
MAX_EMPTY_AXES_BYTES = 20396    # 11 档 figsize 里最大的空坐标轴图
MIN_THIN_REAL_BYTES = 33791     # 本模块所用 figsize 里最薄的真图（9.0x6.0）
# ---------------------------------------------------------------------------
